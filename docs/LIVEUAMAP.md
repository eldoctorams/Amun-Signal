...
AMUN SIGNAL's **Global Incidents** layer does not depend on LiveUAMap. GDELT
...
1. **Supported LiveUAMap API (optional)** — preferred when an operator has a
   paid/contracted API endpoint. AMUN SIGNAL does not require this service.
...
service agreement, AMUN SIGNAL does not hard-code a vendor account endpoint.
...
page representation rather than a stable public schema. AMUN SIGNAL therefore:
...
contained. GDELT fetching and the rest of the AMUN SIGNAL data pipeline continue
independently.
