...
AMUN SIGNAL is **self-hosted**: each install uses its own backend egress IP. This document is the operator-facing record for GitHub audit issues **#348–#366** (tg12): what contacts third parties, why, and how to opt out without losing unrelated features.
...
| **#361** | Mitigated | UA is **install handle only** (`operator-…`), not shared `AMUN SIGNAL/` token |
...
- **Why:** Upstreams can rate-limit **one install**; a block on `operator-abc123` does not require blocking every AMUN SIGNAL user
