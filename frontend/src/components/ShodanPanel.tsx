...
export default function ShodanPanel({
  ...
  const exportPresets = useCallback(() => {
    downloadText(
      `amun-signal-shodan-presets-${new Date().toISOString().slice(0, 10)}.json`,
      JSON.stringify({ source: 'AMUN SIGNAL', type: 'shodan-presets', presets }, null, 2),
    );
  }, [presets]);
  ...
  const exportResultsJson = useCallback(() => {
    ...
    downloadText(
      `amun-signal-shodan-results-${new Date().toISOString().replace(/[:.]/g, '-')}.json`,
      JSON.stringify(
        {
          source: 'Shodan',
  ...
  const exportResultsCsv = useCallback(() => {
    if (!currentResults.length) return;
    downloadText(
      `amun-signal-shodan-results-${new Date().toISOString().replace(/[:.]/g, '-')}.csv`,
