...
export default function AIQueryView({ onBack }: AIQueryViewProps) {
  const [messages, setMessages] = useState<AIMessage[]>([
    {
      role: 'system',
      content: `🌍📡 AMUN SIGNAL AI CO-PILOT ONLINE
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
Connected to AMUN SIGNAL OSINT platform.
...
${pinList || '  No pins placed yet.'}`,
      ...
      if (lowerQuery === 'clear pins') {
        ...
        return { content: '🌍❌ AMUN SIGNAL CLEARING:\nAll AI intel pins cleared.' };
      }
      ...
      if (lowerQuery === 'snapshot' || lowerQuery === 'take snapshot') {
        ...
        const data = await resp.json();
        return {
          content: `🌍🕰️ AMUN SIGNAL TIMEMACHINE:
Snapshot taken!
  🆔 ${data.snapshot_id}
  🕐 ${data.timestamp}
...
Type "help" for the full command list.`,
      ...
      return {
        content: `🌍⚠️ AMUN SIGNAL WARNING:
Query failed: ${error instanceof Error ? error.message : 'Unknown error'}
Make sure the AMUN SIGNAL backend is running on localhost:8000.`,
      };
    }
  }, []);
      ...
      {showConnect && (
        <div className="border-b border-purple-900/40 bg-purple-950/15 px-4 py-4 shrink-0 overflow-y-auto max-h-[60vh]">
          ...
          <div className="space-y-3 text-sm font-mono">
            {/* API Endpoint */}
            <div>
              <div className="text-[11px] text-gray-500 uppercase tracking-widest mb-1">Your AMUN SIGNAL API Endpoint</div>
              <div className="flex items-center gap-2">
            ...
            <div>
              ...
              <div className="bg-black/60 border border-gray-800/40 rounded-sm p-3 space-y-2 text-[13px] leading-relaxed">
                <p className="text-cyan-400 font-bold">Step 1: Install the AMUN SIGNAL Skill</p>
                <p className="text-gray-400">Install the AMUN SIGNAL integration package in your agent&apos;s skills directory.</p>
                ...
                <div className="relative">
                  <pre className="bg-purple-950/40 border border-purple-800/30 px-2 py-2 text-[12px] text-purple-200 rounded-sm overflow-x-auto whitespace-pre-wrap">{`You have an AMUN SIGNAL integration that connects you to a real-time global intelligence platform. Use it to:
...
API: ${apiEndpoint}
Integration docs: docs/AMUN_ARCHITECTURE.md`}</pre>
                  <button
                    onClick={() => handleCopy(`You have an AMUN SIGNAL integration that connects you to a real-time global intelligence platform. Use it to:\n- Query military flights, ships, satellites, SIGINT, earthquakes, and 30+ data layers\n- Place intelligence pins on a live map\n- Fetch satellite imagery from Sentinel-2\n- Aggregate news by region via GDELT\n- Take telemetry snapshots (Time Machine)\n- Participate in the encrypted mesh network\n- Send and receive decentralized intelligence messages\n\nAPI: ${apiEndpoint}\nIntegration docs: docs/AMUN_ARCHITECTURE.md`)}
